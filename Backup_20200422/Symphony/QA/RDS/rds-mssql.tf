resource "aws_db_subnet_group" "kansas_prd_sqlserver" {
  ...
  tags = {
    Name = "${var.tag_name_prefix}-Subnet-Group"
    Project = var.project
    Client = var.client
...
resource "aws_security_group" "kansas_prd_sqlserver" {
  ...
  tags = {
    Name = "${var.tag_name_prefix}-Security-Group"
    Project = var.project
    Client = var.client
    Environment = var.environment
    Zone = var.zone
...
resource "aws_db_option_group" "kansas_prd_sqlserver" {
  ...
  tags = {
    Name = "${var.tag_name_prefix}-Option-Group"
    Project = var.project
    Client = var.client
    Environment = var.environment
    Zone = var.zone
...
**/
  tags = {
    Name = "${var.tag_name_prefix}-Parameter-Group"
    Project = var.project
    Client = var.client
    Environment = var.environment
    Zone = var.zone
