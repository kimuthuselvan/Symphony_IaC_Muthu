resource "aws_iam_role_policy" "static_lb_lambda" {
  name = "lambdaFunction-NLB-to-ALB-policy-${var.default_tags["project_name"]}-${var.default_tags["service_role"]}"
...
resource "aws_iam_role" "static_lb_lambda" {
  name        = "lambdaFunction-NLB-to-ALB-role-${var.default_tags["project_name"]}-${var.default_tags["service_role"]}"
